...
if __name__ == "__main__":
    ...
    users = ["Jeslyn","Bhavya","Aimee","Grace","Anuhya","Allison","Bhrigu","Nathan L","Akhil","Sidharth","Govind","Ryan W","Aiden","Pranaav","Mathew","Jerry","Amaar","Bryan","Nathan W"]
    ...
    for user, match_details in schedule.items():
        ...
        t_H = max(t_H, len(match_details))
        t_L = min(t_L, len(match_details))
        avg += len(match_details)
        if DEBUG_MODE:
    ...
    print(f"\nMax: {t_H}, Min: {t_L}, Avg: {avg/len(users)}, Range: {t_H-t_L}")
